...
public class TurmaController {
    ...
    @GetMapping("/ativas")
    public List<Turma> listarAtivas() { return service.listarAtivas(); }

    @GetMapping("/professor/{professorId}")
    public List<Turma> listarPorProfessor(@PathVariable Long professorId) { return service.listarPorProfessor(professorId); }

    @GetMapping("/curso/{cursoId}")
    public List<Turma> listarPorCurso(@PathVariable Long cursoId) { return service.listarPorCurso(cursoId); }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public Turma criar(@RequestBody Turma turma) { return service.salvar(turma); }

    @PutMapping("/{id}")
    public ResponseEntity<Turma> atualizar(@PathVariable Long id, @RequestBody Turma turma) {
        try { return ResponseEntity.ok(service.atualizar(id, turma)); }
